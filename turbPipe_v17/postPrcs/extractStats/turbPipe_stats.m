%********************************************************************
%  This script computes 44 statistical quantities and their 68 associated derivatives (see below) over a 2D Cartesian mesh specified by './ZSTAT/x.fort' and './ZSTAT/y.fort'.
%  Due to two-dimensionality, the derivatives wrt to z should be zero. 
%    + Reads in './ZSTAT/int_fld', and 
%    - writes out 'data_turbPipe_stat.mat' which contains the statsitical data interpolated over the 2D interpolation mesh.
%  NOTE: No need to modify this script. 
%********************************************************************
%--------------------------------------------------------------------
...
folder='./';
%Read in flow specifications and case setup parameters
geoData=importdata('./tempData/geo.mat');
nR=geoData.nR;   %no. of pts in the radial direction
nTh=geoData.nTh; %no. of pts in the azimuthal direction
nu=geoData.nu;   %Kinematic viscosity
rho=geoData.rho; %Fluid density
nx=nTh;
ny=nR;

% Read in interpolated field: int_fld file
% NOTE: Any modification in the following readings should be accompanied by appropriate modification in ./ppNekOutData/turbPipe_post.usr. 
fsave         = './tempData/data_turbPipe_stat.mat';
fname         = [folder,'../ppNekOutData/ZSTAT/int_fld'];
...
dum6          = fread(fid,1,'*float64')   ;   %dummy

% Read interpolating mesh: x.fort and y.fort
fname_x           = [folder,'../ppNekOutData/ZSTAT/x.fort'];
...
fname_y          = [folder,'../ppNekOutData/ZSTAT/y.fort'];
...
for j=1:ny
    for i=1:nx
        Pi_tensor(:,:,i,j)=[-1/rho*(PT_tensor(1,1,i,j)-PS_tensor(1,1,i,j)) ...
                            ...
                            -1/rho*(PT_tensor(2,2,i,j)-PS_tensor(2,2,i,j)) ...
                            ...
                            -1/rho*(PT_tensor(3,3,i,j)-PS_tensor(3,3,i,j))];
    end
end
...
PTxx=squeeze(squeeze(PT_tensor(1,1,:,:)));
PTyy=squeeze(squeeze(PT_tensor(2,2,:,:)));
PTzz=squeeze(squeeze(PT_tensor(3,3,:,:)));
PTxy=squeeze(squeeze(PT_tensor(1,2,:,:)));
PTxz=squeeze(squeeze(PT_tensor(1,3,:,:)));
PTyz=squeeze(squeeze(PT_tensor(2,3,:,:)));

PSxx=squeeze(squeeze(PS_tensor(1,1,:,:)));
PSyy=squeeze(squeeze(PS_tensor(2,2,:,:)));
PSzz=squeeze(squeeze(PS_tensor(3,3,:,:)));
PSxy=squeeze(squeeze(PS_tensor(1,2,:,:)));
PSxz=squeeze(squeeze(PS_tensor(1,3,:,:)));
PSyz=squeeze(squeeze(PS_tensor(2,3,:,:)));
...
%Plot <Uz> along with the interpolation mesh to check everything is alright .
%Of course you can plot contours of other quantities
figure(1)
pcolor(X,Y,W)
%shading interp;
colorbar;axis equal tight;

%save averaged-interpolated fields
save([fsave],'U','V','W','uu','vv','ww','uv','uw','vw','P','pp','ppp','pppp',...
    'uuu','vvv','www','uuv','uuw','uvv','vvw','uww','vww','uvw',...
    'Pxx','Pyy','Pzz','Pxy','Pxz','Pyz',...
    'Dxx','Dyy','Dzz','Dxy','Dxz','Dyz',...
    'Txx','Tyy','Tzz','Txy','Txz','Tyz',...
    'VDxx','VDyy','VDzz','VDxy','VDxz','VDyz',...
    'Pixx','Piyy','Pizz','Pixy','Pixz','Piyz',...
    'Cxx','Cyy','Czz','Cxy','Cxz','Cyz',...
    'PTxx','PTyy','PTzz','PTxy','PTxz','PTyz',...
    'PSxx','PSyy','PSzz','PSxy','PSxz','PSyz',...
    'dUdx','dUdy','dUdz','dVdx','dVdy','dVdz','dWdx','dWdy','dWdz','nu');
